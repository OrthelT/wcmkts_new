...
| `price_service.py` | `JitaPriceService` -- resolves Jita prices from `DatabasePriceProvider` (backend-populated price table) only. A `FallbackPriceProvider` chain (DB cache -> Fuzzwork -> Janice) exists but the live-API providers are intentionally not wired into the default chain; they can be re-enabled if API fallback is ever needed. Single batch entry: `get_jita_prices(type_ids) -> BatchPriceResult` |
