...
| `price_service.py` | `JitaPriceService` -- currently resolves to DatabasePriceProvider only. However, a provider chain exists, but is currently disabled, that allow for fallback to API calls if the database Jita price table is empty (DB cache -> (CURRENTLY DISABLED: Fuzzwork -> Janice) using `FallbackPriceProvider`; single batch entry `get_jita_prices(type_ids) -> BatchPriceResult` | this can be re-enabled if there is need for API calls to resolve Jita prices. 
