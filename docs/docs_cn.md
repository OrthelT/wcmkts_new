...
**多语言支持：** 应用支持8种语言（英语、中文、德语、法语、俄语、西班牙语、日语、韩语）。在应用右上角使用语言选择器切换语言。您的选择通过 URL 查询参数（`?lang=xx`）持久保存，方便收藏特定语言的链接。
...
### 3. 进货助手页面
**目的：** 帮助识别本地市场价格明显高于吉他卖价的物品，便于快速发现潜在进货机会。

**主要功能：**
- **吉他价格对比：** 对比本地价格、吉他卖价和吉他买价
- **运输成本输入：** 可配置的每 m³ 运输成本（默认 450 ISK/m³）；在页面顶部的数字输入框中调整
- **30天相对吉他卖价利润：** 按 `(本地价格 - 吉他卖价) * 平均日成交量 * 30` 计算
- **30天交易额：** 按本地近30天平均日成交量乘以30，再乘以吉他卖价估算
- **30天成交量：** 按本地平均日成交量乘以30估算
- **资本利用率：** 按 `((本地价格 - 吉他卖价) - 运输成本) / 吉他卖价` 计算

**使用方法：**
1. 使用侧边栏按类别或物品名称筛选
2. 根据您的实际运费调整每 m³ 运输成本输入框
3. 对比本地价格与吉他卖价/买价
4. 结合运输成本、30天利润和资本利用率一起判断
5. 参考30天交易额和30天成交量列，优先关注成交额较高的物品
...
## 数据库同步
会话打开期间，应用会自动轮询并与远程 EVE Online 市场数据库同步，两次同步之间至少间隔 600 秒（见 `pages/components/db_refresh.py`）。该间隔为硬编码，`settings.toml` 中没有可调整的配置项。您也可以在侧边栏点击“立即同步”手动触发同步。如有新数据，将自动更新。

**同步状态指示：**
- 上次 ESI 更新：显示从 ESI 获取市场数据的时间  
- 上次同步：显示本地数据库最后一次与远程数据库同步的时间  
- 下次更新（估计）：以最后一次 `updatelog` 时间戳加 60 分钟推算**后端**下次发布数据的时间（`state/sync_state.py` 中的 `_UPDATE_INTERVAL_MINUTES`），并非本应用的同步计划  
- 状态：显示最近一次同步的成功或失败  
